...
interface Gpc {
     ...
     * 2. If the remote configuration has enabled or not the GPC feature.
     * When disabled, the remote configuration value prevails over the user choice.
     * @return `true` if the feature is enabled and `false` is is not.
     */
    fun isEnabled(): Boolean
    /**
     * This method returns a [Map] with the GPC headers IF the url passed
    ...
    fun enableGpc()
    /**
     * This method is used to disable GPC.
     * Note: The remote configuration will take precedence over this value.
     */
    fun disableGpc()
}
