...
class PrivacyFeatureTogglesPlugin @Inject constructor(privacyConfigDatabase: PrivacyConfigDatabase, private val dispatcherProvider: DispatcherProvider) :
    ...
    override fun isEnabled(featureName: FeatureName, defaultValue: Boolean): Boolean? {
        return runBlocking(dispatcherProvider.io()) {
            if (featureName is PrivacyFeatureName) {
                val privacyFeature: PrivacyFeatureToggles? = privacyFeatureTogglesDao.get(featureName.value)
                privacyFeature?.let {
