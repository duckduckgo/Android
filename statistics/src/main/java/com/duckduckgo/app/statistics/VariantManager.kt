...
interface VariantManager {
    ...
    companion object {
        ...
        val ACTIVE_VARIANTS =
            listOf(
                // SERP variants. "sc" may also be used as a shared control for mobile experiments
                // in
                ...
                Variant(
                    ...
                    weight = 0.0,
                    features = emptyList(),
                    filterBy = { isSerpRegionToggleCountry() }),
                Variant(
                    key = "se",
                    weight = 0.0,
                    features = emptyList(),
                ...
                Variant(
                    key = "mj",
                    weight = 0.0,
                    features = listOf(VariantFeature.FireproofExperiment),
                    filterBy = { isEnglishLocale() }),
                ...
                Variant(
                    key = "zv",
                    weight = 1.0,
                    features = listOf(VariantFeature.ReturningUsersNoOnboarding),
                    filterBy = { isEnglishLocale() }),
                Variant(
                    key = "zz",
                    weight = 1.0,
                    features = listOf(VariantFeature.ReturningUsersWidgetPromotion),
                    filterBy = { isEnglishLocale() }),
            )

        val REFERRER_VARIANTS =
            listOf(
                Variant(
                    RESERVED_EU_AUCTION_VARIANT, features = emptyList(), filterBy = { noFilter() }))

        private val serpRegionToggleTargetCountries =
            listOf("AU", "AT", "DK", "FI", "FR", "DE", "IT", "IE", "NZ", "NO", "ES", "SE", "GB")
...
class ExperimentationVariantManager(
    ...
    override fun getVariant(activeVariants: List<Variant>): Variant {
        ...
        if (currentVariant == null) {
            Timber.i(
                "Variant $currentVariantKey no longer an active variant; user will now use default variant")
...
fun VariantManager.isFireproofExperimentEnabled() =
    this.getVariant().hasFeature(VariantManager.VariantFeature.FireproofExperiment)

fun VariantManager.returningUsersNoOnboardingEnabled() =
    this.getVariant().hasFeature(VariantManager.VariantFeature.ReturningUsersNoOnboarding)

fun VariantManager.returningUsersWidgetPromotionEnabled() =
    this.getVariant().hasFeature(VariantManager.VariantFeature.ReturningUsersWidgetPromotion)

/**
 * A variant which can be used for experimentation.
 * @param weight Relative weight. These are normalised to all other variants, so they don't have to
 * add up to any specific number.
 */
data class Variant(
    val key: String,
