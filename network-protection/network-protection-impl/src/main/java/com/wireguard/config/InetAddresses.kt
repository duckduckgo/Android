    ...
    fun parse(address: String): InetAddress {
        ...
        return try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) InetAddresses.parseNumericAddress(
                address
            ) else if (PARSER_METHOD != null) PARSER_METHOD!!.invoke(
                null,
                address
            ) as InetAddress else throw NoSuchMethodException("parseNumericAddress")
            ...
            try {
                if (WONT_TOUCH_RESOLVER.matcher(address)
                        .matches()
                ) InetAddress.getByName(address) else throw ParseException(
                    InetAddress::class.java, address, "Not an IP address"
                )
