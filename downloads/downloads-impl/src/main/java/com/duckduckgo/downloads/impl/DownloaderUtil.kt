...
object DownloaderUtil {
    ...
    private val ENCODED_FILENAME_PATTERN =
        Pattern.compile(
            "\\s*;?\\s*filename\\*\\s*=\\s*([a-z0-9\\-]+)'\\s*'$FILENAME_PATTERN",
            Pattern.CASE_INSENSITIVE,
        ).toRegex()

    private val PLAIN_FILENAME_PATTERN =
        Pattern.compile(
    ...
    fun fileNameFromContentDisposition(contentDisposition: String): String? {
        val filename = try {
            parseEncodedFilenameContentDisposition(removePlainFilenamePattern(contentDisposition))
                ?: parseContentDisposition(contentDisposition) ?: return null
        } catch (ex: UnsupportedEncodingException) {
            parseContentDisposition(removeEncodedFilenamePattern(contentDisposition)) ?: return null
        }
    ...
    private fun removeEncodedFilenamePattern(contentDisposition: String): String {
        return contentDisposition.replace(ENCODED_FILENAME_PATTERN, "")
    }

    private fun removePlainFilenamePattern(contentDisposition: String): String {
        return contentDisposition.replace(PLAIN_FILENAME_PATTERN, "")
    }
