...
val flowId: Long = wideEventClient.flowStart(
    ...
    samplingProbability = 0.1f,           // optional: 0.0–1.0, default 1.0 (keep all)
).getOrNull() ?: return
...
`samplingProbability` controls what fraction of flows are recorded. The decision is made once at `flowStart` time:

- **1.0** (default) — every flow is recorded.
- **0.1** — roughly 10% of flows are recorded; the rest are dropped.
- **0.0** — no flows are recorded.
