...
/**
 * Provides the appropriate StartupCollector implementation based on device API level.
 */
@ContributesBinding(AppScope::class)
class StartupCollectorProvider @Inject constructor(
    ...
    private val delegate: StartupCollector by lazy {
        if (Build.VERSION.SDK_INT >= 35) {
            Api35StartupCollector(
                context = context,
    ...
    override suspend fun collectStartupMetrics(startupType: StartupType): StartupMetricEvent? =
        delegate.collectStartupMetrics(startupType)
}
