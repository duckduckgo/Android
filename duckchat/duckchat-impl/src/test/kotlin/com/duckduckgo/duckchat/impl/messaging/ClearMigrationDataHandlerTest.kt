...
class ClearMigrationDataHandlerTest {
    ...
    @Test
    fun `when clear migration data then items are removed`() {
        standaloneStore.storeMigrationItem("Blob1")
        standaloneStore.storeMigrationItem("Blob2")
        assertEquals(2, standaloneStore.getMigrationItemCount())
    ...
    @Test
    fun `when clear migration data and no id then do nothing`() {
        val message = JsMessage(
            context = "context",
            featureName = handler.featureName,
            method = handler.methods.first(),
    ...
    fun `only contains valid methods`() {
        ...
        assertTrue(methods[0] == "clearMigrationData")
    }
}
