...
class DeviceShieldNotificationFactory
@Inject
constructor(
    ...
    @VisibleForTesting val dailyNotificationFactory = DeviceShieldDailyNotificationFactory()

    @VisibleForTesting val weeklyNotificationFactory = DeviceShieldWeeklyNotificationFactory()
    ...
    fun createNotificationNewTrackerFound(
        trackersBlocked: List<VpnTracker>
        ...
        val numberOfAppsString =
            resources.getQuantityString(
                R.plurals.atp_NotificationNumberOfApps, numberOfApps.size, numberOfApps.size)
        ...
        return DeviceShieldNotification(
            notificationText.applyBoldSpanTo(listOf(numberOfAppsString)))
    }
    ...
    private fun getNumberOfAppsContainingTopOffender(
        ...
        return trackers.filter { it.trackerCompanyId == topOffender.trackerCompanyId }.groupBy {
            it.trackingApp
        }
    }
    ...
    inner class DeviceShieldDailyNotificationFactory {
        ...
        suspend fun createDailyDeviceShieldNotification(
            dailyNotificationType: Int
        ): DeviceShieldNotification {

            val trackers = appTrackerBlockingStatsRepository.getVpnTrackersSync({ dateOfLastDay() })
            val trackerCount =
                appTrackerBlockingStatsRepository
                    .getBlockedTrackersCountBetween({ dateOfLastDay() })
                    .firstOrNull()
                    ?: trackers.size
            Timber.v(
                "createDailyDeviceShieldNotification. $trackerCount total trackers in the last day, number of trackers returned is ${trackers.size}. Notification type: $dailyNotificationType")
            ...
            val apps =
                trackers.groupBy { it.trackingApp }.toList().sortedByDescending { it.second.size }
        ...
        private fun createDailyTotalTrackersNotification(
            ...
            val totalTrackers =
                resources.getQuantityString(
                    R.plurals.atp_TrackingAttempts, totalTrackersCount, totalTrackersCount)
            val textPrefix = resources.getString(R.string.atp_DailyTrackersNotificationPrefix)
            val numberTrackers =
                resources.getQuantityString(
                    R.plurals.atp_TrackingAttempts, totalTrackersCount, totalTrackersCount)
            val optionalNumberApps =
                if (apps == 0) ""
                else {
                    resources.getQuantityString(
                        R.plurals.atp_DailyTrackersNotificationSuffixNumApps,
                        apps,
                        apps,
                        firstAppName)
                }
            ...
            Timber.v(
                "createDailyTotalTrackersNotification. Trackers=$totalTrackers. Apps=$apps. Output=[$textToStyle]")
            return DeviceShieldNotification(
                textToStyle.applyBoldSpanTo(
                    listOf(totalTrackers, numberTrackers, optionalNumberApps)))
        }

        private fun createDailyTopTrackerCompanyNotification(
            trackers: List<VpnTracker>
        ): DeviceShieldNotification {
            ...
            val prefix =
                resources.getString(
                    R.string.atp_DailyTopCompanyNotificationPrefix, topOffender.companyDisplayName)
            val numAppsText =
                resources.getQuantityString(
                    R.plurals.atp_NotificationNumberOfApps, numberApps, numberApps)
            ...
            Timber.v("createDailyTopTrackerCompanyNotification: $fullString")
            return DeviceShieldNotification(
                fullString.applyBoldSpanTo(
                    listOf(topOffender.companyDisplayName, numAppsText, seeMoreSuffix)))
        }

        private fun createDailyNotificationTopAppsContainingTrackers(
            apps: List<Pair<TrackingApp, List<VpnTracker>>>
        ): DeviceShieldNotification {
            if (apps.isEmpty()) {
                return DeviceShieldNotification(hidden = true)
            }
            ...
            val prefix =
                resources.getString(
                    R.string.atp_DailyCompanyBlockedNotificationPrefix, firstAppName)
            val optionalSecondApp =
                if (second != null)
                    resources.getString(
                        R.string.atp_DailyCompanyBlockedNotificationOptionalSecondApp,
                        second.appDisplayName)
                else ""
            ...
            Timber.v(
                "createDailyNotificationTopAppsContainingTrackers. Text to style: [$textToStyle] Words to bold: ${wordsToBold.joinToString(separator = ", ")}}")
            return DeviceShieldNotification(textToStyle.applyBoldSpanTo(wordsToBold))
        }

        private fun createDailyLastCompanyAttemptNotification(
            trackers: List<VpnTracker>
        ): DeviceShieldNotification {
            val lastCompany = trackers.first()
            val latestApp = lastCompany.trackingApp.appDisplayName
            val filteredForLatestTrackerCompany =
                trackers.filter { it.trackerCompanyId == lastCompany.trackerCompanyId }
            val timesBlocked = filteredForLatestTrackerCompany.size
            val appsContainingLatestTracker =
                filteredForLatestTrackerCompany.groupBy { it.trackingApp }

            val prefix =
                resources.getString(
                    R.string.atp_DailyLastCompanyBlockedNotification,
                    lastCompany.companyDisplayName)
            val numberOfTimesString =
                resources.getQuantityString(R.plurals.atp_NumberTimes, timesBlocked, timesBlocked)
            val latestAppString =
                resources.getString(
                    R.string.atp_DailyLastCompanyBlockedNotificationInApp, latestApp)
            ...
            val textToStyle =
                "$prefix $numberOfTimesString $latestAppString$otherAppsCount $pastDaySuffix"
    ...
    inner class DeviceShieldWeeklyNotificationFactory {
        ...
        suspend fun createWeeklyDeviceShieldNotification(
            randomNumber: Int
        ): DeviceShieldNotification {
            val trackers =
                appTrackerBlockingStatsRepository.getVpnTrackersSync({ dateOfLastWeek() })
            val trackerCount =
                appTrackerBlockingStatsRepository
                    .getBlockedTrackersCountBetween({ dateOfLastDay() })
                    .firstOrNull()
        ...
        private fun createWeeklyReportNotification(
            ...
            val totalTrackers =
                resources.getQuantityString(
                    R.plurals.atp_TrackingAttempts, trackerCount, trackerCount)
            val optionalOtherAppsString =
                if (otherAppsSize == 0) ""
                    ...
                    resources.getQuantityString(
                        ...
                        otherAppsSize)
            val pastWeekSuffix =
                resources.getString(R.string.atp_WeeklyCompanyTeaserNotificationSuffix)

            val textToStyle =
                "$prefix $totalTrackers $latestAppString$optionalOtherAppsString$pastWeekSuffix"

            Timber.v(
                "createWeeklyReportNotification. $textToStyle\nTotal apps: ${perApp.size}. Other apps: $otherAppsSize")

            return DeviceShieldNotification(
                textToStyle.applyBoldSpanTo(listOf(totalTrackers, latestAppString)))
        }

        private fun createWeeklyTopTrackerCompanyNotification(
            trackers: List<VpnTracker>
        ): DeviceShieldNotification {
            ...
            val appsContainingTrackerEntity =
                getNumberOfAppsContainingTopOffender(trackers, topOffender.first())
            val prefixString =
                resources.getString(R.string.atp_WeeklyCompanyTeaserNotificationPrefix, company)
            val numberOfApps = appsContainingTrackerEntity.size
            val mostRecentAppContainingTracker =
                trackers.firstOrNull { it.trackerCompanyId == topOffender.first().trackerCompanyId }
                    ?: return DeviceShieldNotification(hidden = true)

            val numberOfAppsString =
                resources.getQuantityString(
                    R.plurals.atp_NotificationNumberOfApps, numberOfApps, numberOfApps)
            ...
            return DeviceShieldNotification(
                textToStyle.applyBoldSpanTo(
                    listOf(company, numberOfAppsString, mostRecentAppString)))
        }
    }
