...
class DomainBasedTrackerDetector(
    ...
    override fun determinePacketType(
        ...
        if (isLocalAddress) {
            Timber.v(
                "%s is a local address; not looking for trackers",
        ...
        if (hostname == null) {
            Timber.w(
                "Failed to determine if packet is a tracker as hostname not extracted %s",
                tcb.ipAndPort)
            return RequestTrackerType.Undetermined
        }

        for (interceptor in requestInterceptors.getPlugins()) {
            interceptor.interceptTrackerRequest(hostname, requestingApp.packageId)?.let {
                return it
            }
        }

        return when (val trackerType =
            appTrackerRepository.findTracker(hostname, requestingApp.packageId)) {
            is FirstParty -> {
                Timber.d(
            ...
            is ThirdParty -> {
                ...
                return if (isTrackerInExceptionRules(trackerModel, requestingApp)) {
                    ...
                    when (tlsContentTypeExtractor.isTlsApplicationData(payloadBytes)) {
                        Undetermined -> {
                            Timber.v(
                                "Unable to determine TLS content type, fallback to blocking as if it were application data %s",
                                tcb.ipAndPort)
                            tcb.trackerTypeDetermined = true
                            tcb.isTracker = true
                        ...
                        TlsApplicationData -> {
                            Timber.v(
                                "This is TLS content type: Application Data and is a 3rd party tracker. We'll block this. %s",
                                tcb.ipAndPort)
                            tcb.trackerTypeDetermined = true
                            tcb.isTracker = true
                            recordTrackerBlocked(trackerType, tcb, requestingApp)
                        ...
                        NotApplicationData -> {
                            // this is request for a tracker, but we don't want to block the TLS
                            // handshake, so treat this as a non-tracker for now
                            tcb.trackerTypeDetermined = false
                            Timber.v(
                                "This is a TLS message but isn't ApplicationData. We'll allow this for now %s",
                                tcb.ipAndPort)
    ...
    private fun isTrackerInExceptionRules(
        ...
        val exceptionRule =
            vpnDatabase.vpnAppTrackerBlockingDao().getRuleByTrackerDomain(tracker.hostName)

        return exceptionRule != null &&
            exceptionRule.packageNames.contains(originatingApp.packageId)
    }
    ...
    private fun insertTracker(tracker: AppTracker, requestingApp: OriginatingApp) {
        if (requestingApp.isInvalid()) {
            ...
            // we don't yet know the reason why the DDG app appears sometimes in the list of of
            // tracking apps
            // for now we manually exclude while we investigate the root cause
            Timber.d(
                "Originating app is either DDG or UNKNOWN, skipping db insertion for %s (%s)",
                requestingApp.appName,
                requestingApp.packageId)
            return
        }
