...
class TcpStateFlow {
    ...
    companion object {
        fun newPacket(
            ...
            val newActions =
                when (currentState.serverState) {
                    LISTEN -> handlePacketInStateListen(connectionKey, currentState, packetType)
                    SYN_RECEIVED ->
                        handlePacketInSynReceived(
                            connectionKey, currentState, packetType, sequenceNumberToClientInitial)
                    ESTABLISHED ->
                        handlePacketInEstablished(connectionKey, currentState, packetType)
            ...
            Timber.v(
                "%s. [%s]. New actions are [%s]",
                connectionKey,
                currentState,
                newActions.ifEmpty { listOf("No Actions") }.joinToString())

            return TcpStateAction(newActions)
        }

        private fun handlePacketInTimeWait(
            connectionKey: String,
            packetType: PacketType
        ): List<Event> {
            val events =
                when {
                    packetType.isRst -> {
                        Timber.w(
                            "Received RESET while in TIME_WAIT. Closing connection %s",
                            connectionKey)
        ...
        private fun handlePacketInClosed(
            connectionKey: String,
            packetType: PacketType
        ): List<Event> {
            val events =
                when {
                    packetType.isRst -> {
                        Timber.w(
                            "Received RESET while in CLOSED. Closing connection %s", connectionKey)
                        listOf(CloseConnection)
                    }
                    else -> listOf(SendReset)
                }
        ...
        private fun handlePacketInLastAck(
            packetType: PacketType,
            connectionKey: String,
            currentState: TcbState
        ): List<Event> {
            val events =
                when {
                    packetType.isRst -> {
                        Timber.w(
                            "Received RESET while in %s. Closing connection %s",
                            currentState,
                            connectionKey)
                        listOf(CloseConnection)
                    }
        ...
        private fun isMatchingAckForOurFin(packetType: PacketType): Boolean {
            ...
            val difference =
                (packetType.ackNum - packetType.finSequenceNumberToClient).absoluteValue
            return (difference <= 1)
        }

        private fun handlePacketInFinWait1(
            connectionKey: String,
            currentState: TcbState,
            packetType: PacketType
        ): List<Event> {
            ...
            eventList.addAll(
                when {
                    ...
                    packetType.isAck -> {
                        // we can also arrive here if the server ends the stream and we send a FIN
                        // to client
        ...
        private fun handlePacketInFinWait2(
            connectionKey: String,
            packetType: PacketType
        ): List<Event> {
            val eventList = mutableListOf<Event>()
            ...
            eventList.addAll(
                when {
                    packetType.isFin -> {
                        listOf(
                            SendAck,
                            MoveServerToState(CLOSING),
                            MoveClientToState(TIME_WAIT),
                            DelayedCloseConnection)
                    }
                    packetType.isRst -> {
                        Timber.w(
                            "%s - Received RESET while in FIN_WAIT_2. Closing connection",
                            connectionKey)
                        listOf(CloseConnection)
                    }
                    packetType.hasData -> {
                        ...
                        Timber.w(
                            "%s - Received packet while in FIN_WAIT_2 - ignoring", connectionKey)
                        emptyList()
                    }
                })
        ...
        private fun handlePacketInClosing(
            connectionKey: String,
            currentState: TcbState,
            packetType: PacketType
        ): List<Event> {
                ...
                when {
                    ...
                    packetType.isSyn -> {
                        if (currentState.clientState == SYN_SENT) {
                            Timber.d(
                                "Opening a connection when SYN already sent; duplicate SYN can be ignored. %s",
                                connectionKey)
                            return emptyList()
                        }
                    ...
                    packetType.hasData -> listOf(SendAck)
                        ...
                        Timber.w(
                            "%s - Received packet in LISTEN state, but not a SYN or RST.",
                            connectionKey)
                        listOf(SendReset)
                    }
                }
            ...
            val events =
                when {
                    ...
                    packetType.isAck -> {
                        ...
                        listOf(
                            MoveServerToState(ESTABLISHED),
                            MoveClientToState(ESTABLISHED),
                            ProcessPacket)
                    }
                    else -> listOf(SendReset)
                }
        ...
        private fun handlePacketInEstablished(
            connectionKey: String,
            currentState: TcbState,
            packetType: PacketType
        ): List<Event> {
                ...
                when {
                    packetType.isFin -> {
                        mutableListOf<Event>().also { events ->
                            // FIN might also have data, so ProcessPacket will handle that. if not,
                            // still need to send ACK in response to FIN
                            if (packetType.hasData) {
        ...
        fun socketEndOfStream(): TcpStateAction {
            val deferredStateMoves =
                listOf(MoveServerToState(FIN_WAIT_1), MoveClientToState(LAST_ACK))
            return TcpStateAction(listOf(SendDelayedFin(deferredStateMoves)))
        }

        private fun unhandledEvent(
            connectionKey: String,
            currentState: TcbState,
            packetType: PacketType
        ): List<Event> {
