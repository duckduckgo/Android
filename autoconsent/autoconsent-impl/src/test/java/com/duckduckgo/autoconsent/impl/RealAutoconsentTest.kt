...
class RealAutoconsentTest {
    ...
    fun setup() {
        ...
        whenever(mockVariantManager.getVariant()).thenReturn(ACTIVE_VARIANTS.first { it.key == "mb" })
    }
    ...
    fun whenInjectAutoconsentAndCookiePromptManagementExperimentIsEnabledAndAutoconsentIsDisabledAndAlreadyHandledThenDoNothing() {
        whenever(mockVariantManager.getVariant()).thenReturn(ACTIVE_VARIANTS.first { it.key == "mc" })
