            ...
            entries.forEach { entry ->
                runBlocking(dispatchers.io()) {
                    ...
                    if (localCredentials == null) {
                        ...
                        val remoteCredentials = syncCredentialsMapper.toLoginCredential(entry, localId)
                        if ((remoteCredentials.lastUpdatedMillis ?: 0L) > (localCredentials.lastUpdatedMillis ?: 0L)) {
                            Timber.d("Sync-autofill-Persist: >>> update with remote $remoteCredentials")
                            autofillStore.updateCredentials(remoteCredentials)
                        }
                    }
                }
            }
