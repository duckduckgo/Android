...
class RealSecureStorageRepositoryTest {
    ...
    private lateinit var db: com.duckduckgo.autofill.store.db.SecureStorageDatabase

    private lateinit var websiteLoginCredentialsDao: com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsDao
    private lateinit var neverSavedSitesDao: com.duckduckgo.autofill.store.db.NeverSavedSitesDao
    private lateinit var testee: com.duckduckgo.autofill.store.RealSecureStorageRepository

    @Before
    fun setUp() {
        MockitoAnnotations.openMocks(this)
        db = Room.inMemoryDatabaseBuilder(RuntimeEnvironment.getApplication(), com.duckduckgo.autofill.store.db.SecureStorageDatabase::class.java)
    ...
    fun whenRetrievingLoginCredentialByDomainThenReturnedIfDirectMatch() = runTest {
        ...
        val result: List<com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsEntity> = testee.websiteLoginCredentialsForDomain("test.com").first()
    ...
    fun whenRetrievingLoginCredentialByEmptyDomainThenReturnedIfDirectMatch() = runTest {
        ...
        websiteLoginCredentialsDao.insert(testEntity)
        val result: List<com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsEntity> = testee.websiteLoginCredentialsForDomain("").first()
        assertEquals(testEntity, result[0])
    }
    ...
    fun whenRetrievingLoginCredentialByDomainThenEmptyListReturnedIfNoMatches() = runTest {
        val testEntity = entity()
        websiteLoginCredentialsDao.insert(testEntity)
        val result: List<com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsEntity> = testee.websiteLoginCredentialsForDomain("no-matches.com").first()
        assertTrue(result.isEmpty())
    }

    @Test
    fun whenGetAllWebsiteLoginCredentialsWithSitesThenEmptyListReturned() = runTest {
        val result: List<com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsEntity> = testee.websiteLoginCredentials().first()
        assertTrue(result.isEmpty())
    }
    ...
    fun whenNeverSavedSiteAddedThenCountIncreases() = runTest {
        testee.neverSaveListCount().test {
            assertEquals(0, awaitItem())
            neverSavedSitesDao.insert(com.duckduckgo.autofill.store.db.NeverSavedSiteEntity(domain = "test.com"))
    ...
    ): com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsEntity {
        return com.duckduckgo.autofill.store.db.WebsiteLoginCredentialsEntity(
            id = id,
            domain = domain,
            username = username,
            password = password,
