...
class WebMessageListenerGetAutofillData @Inject constructor(
    ...
    private suspend fun getAutofillData(requestString: String, autofillUrlRequest: AutofillUrlRequest) {
        Timber.v("BrowserAutofill: getAutofillData called:\n%s", requestString)
        if (autofillUrlRequest.originalPageUrl == null) {
            Timber.w("Can't autofill as can't retrieve current URL")
            return
        }

        if (!autofillCapabilityChecker.canInjectCredentialsToWebView(autofillUrlRequest.requestOrigin)) {
        ...
        if (request.mainType != CREDENTIALS) {
            handleUnknownRequestMainType(request, autofillUrlRequest)
            return
        }

        if (request.isGeneratedPasswordAvailable()) {
            handleRequestForPasswordGeneration(autofillUrlRequest, request)
        } else if (request.isAutofillCredentialsRequest()) {
            handleRequestForAutofillingCredentials(autofillUrlRequest, request, triggerType)
        } else {
    ...
    private suspend fun handleRequestForPasswordGeneration(
        ...
        callback.onGeneratedPasswordAvailableToUse(autofillUrlRequest, request.generatedPassword?.username, request.generatedPassword?.value!!)
    }
    ...
    private fun handleUnknownRequestMainType(
        ...
        onNoCredentialsAvailable(autofillUrlRequest)
    }

    private suspend fun handleRequestForAutofillingCredentials(
        urlRequest: AutofillUrlRequest,
        request: AutofillDataRequest,
    ...
    private fun onNoCredentialsAvailable(urlRequest: AutofillUrlRequest) {
