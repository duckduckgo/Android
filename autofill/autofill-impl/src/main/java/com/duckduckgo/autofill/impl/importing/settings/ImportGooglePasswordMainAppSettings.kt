...
class ImportGooglePasswordMainAppSettings @Inject constructor(
    ...
    override suspend fun canShow(): Boolean {
        return promoRules.canShowPromo()
    }

    override fun getView(activity: AppCompatActivity, onStateChangedCallback: () -> Unit): View? {
        return OneLineListItem(activity).apply {
            ...
            setOnClickListener {
                onLaunchImportFlow(activity, onStateChangedCallback)
            }

            configureOverflowMenu(onStateChangedCallback)
        }
    }

    private fun onLaunchImportFlow(activity: AppCompatActivity, onStateChangedCallback: () -> Unit) {
        listenForImportResult(activity, onStateChangedCallback)
    ...
    private fun listenForImportResult(activity: AppCompatActivity, onStateChangedCallback: () -> Unit) {
    ...
    private fun OneLineListItem.configureOverflowMenu(onStateChanged: () -> Unit) {
        showTrailingIcon()
        setTrailingIconClickListener { overflowView ->
            val layoutInflater = LayoutInflater.from(context)
            val popupMenu = buildPopupMenu(this, layoutInflater, onStateChanged)
    ...
    private fun buildPopupMenu(
        ...
        popupMenu.apply {
            onMenuItemClicked(hideButton) {
                rootView.findViewTreeLifecycleOwner()?.lifecycleScope?.launch(dispatchers.io()) {
