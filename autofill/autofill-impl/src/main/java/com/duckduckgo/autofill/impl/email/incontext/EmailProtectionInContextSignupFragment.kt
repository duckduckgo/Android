...
class EmailProtectionInContextSignupFragment :
    ...
    @Inject
    lateinit var messagePoster: AutofillMessagePoster
    ...
    private suspend fun onFlowEnded() {
        withContext(dispatchers.io()) {
            val requestId = getMessageRequestId()
            ...
            messagePoster.postMessage(message, requestId)
        }
    }
    ...
    private suspend fun cancelInContextSignUp() {
        activity?.let {
            onFlowEnded()
            it.setResult(EmailProtectionInContextSignUpScreenResult.CANCELLED)
            it.finish()
        }
    }
    ...
    private suspend fun closeActivityAsSuccessfulSignup() {
        activity?.let {
            onFlowEnded()
            it.setResult(EmailProtectionInContextSignUpScreenResult.SUCCESS)
            it.finish()
        }
    }
