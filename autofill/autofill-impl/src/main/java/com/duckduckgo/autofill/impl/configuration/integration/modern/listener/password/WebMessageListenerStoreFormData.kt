...
class WebMessageListenerStoreFormData @Inject constructor(
    ...
    override fun onPostMessage(
        ...
        appCoroutineScope.launch(dispatchers.io()) {
            ...
            storeFormData(
                ...
                AutofillUrlRequest(requestOrigin = requestOrigin, originalPageUrl = originalUrl, requestId = requestId),
            )
        }
    }
    ...
    private suspend fun storeFormData(
        ...
        if (autofillUrlRequest.originalPageUrl == null) return

        if (!autofillCapabilityChecker.canSaveCredentialsFromWebView(autofillUrlRequest.requestOrigin)) {
            Timber.v("BrowserAutofill: storeFormData called but feature is disabled")
            return
        }

        if (neverSavedSiteRepository.isInNeverSaveList(autofillUrlRequest.requestOrigin)) {
        ...
        val credentials = jsCredentials.asLoginCredentials(autofillUrlRequest.requestOrigin)
        ...
        processStoreFormDataActions(actions, autofillUrlRequest, credentials)
    }
    ...
    private suspend fun processStoreFormDataActions(
        ...
        actions.forEach {
            when (it) {
                ...
                is PromptToSave -> {
                    callback.onCredentialsAvailableToSave(autofillUrlRequest, credentials)
                }
