...
class WebMessageListenerShowInContextEmailProtectionSignupPrompt @Inject constructor(
    ...
    override fun onPostMessage(
        ...
        job += appCoroutineScope.launch(dispatchers.io()) {
            ...
            val autofillUrlRequest = AutofillUrlRequest(
                requestOrigin = requestOrigin,
                originalPageUrl = originalUrl,
                requestId = requestId,
            )
            showInContextEmailProtectionSignupPrompt(autofillUrlRequest)
        }
    }

    private fun showInContextEmailProtectionSignupPrompt(autofillUrlRequest: AutofillUrlRequest) {
        appCoroutineScope.launch(dispatchers.io()) {
            val isSignedIn = emailManager.isSignedIn()

            withContext(dispatchers.main()) {
                if (isSignedIn) {
                    callback.showNativeChooseEmailAddressPrompt(autofillUrlRequest)
                } else {
                    callback.showNativeInContextEmailProtectionSignupPrompt(autofillUrlRequest)
                }
            }
        }
    }
