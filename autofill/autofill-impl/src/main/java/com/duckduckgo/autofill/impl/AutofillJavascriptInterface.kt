...
class AutofillStoredBackJavascriptInterface @Inject constructor(
    ...
    override fun getAutofillData(requestString: String) {
        ...
        getAutofillDataJob += coroutineScope.launch(dispatcherProvider.default()) {
            ...
            } else if (request.subType == USERNAME || request.subType == PASSWORD) {
    ...
    private suspend fun handleRequestForPasswordGeneration(
        ...
        callback?.onGeneratedPasswordAvailableToUse(url, request.generatedPassword?.value!!)
    }
    ...
    fun storeFormData(data: String) {
        ...
        storeFormDataJob += coroutineScope.launch(dispatcherProvider.default()) {
            ...
            val actions = passwordEventResolver.decide(autosavedLogin, request.credentials.autogenerated)
    ...
    private suspend fun processStoreFormDataActions(
        ...
        actions.forEach {
            when (it) {
                ...
                SilentlySave -> {
                    autofillStore.saveCredentials(currentUrl, credentials)?.id?.let { newLoginId ->
                        autoSavedLoginsMonitor?.setAutoSavedLoginId(newLoginId, tabId)
                        Timber.d("Quietly saved credentials which were autogenerated. id=%s", newLoginId)
                    }
                }
                ...
                is UpdateSavedAutoLogin -> {
                    autofillStore.getCredentialsWithId(it.autologinId)?.let { existingCredentials ->
                        ...
                        autofillStore.updateCredentials(toSave)?.let { savedCredentials ->
                            Timber.d("Quietly updated credentials which were autogenerated. id=%s", autologinId)
