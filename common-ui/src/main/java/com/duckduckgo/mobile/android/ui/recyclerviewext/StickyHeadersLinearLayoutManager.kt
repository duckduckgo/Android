 ...
 *
 * Adds sticky headers capabilities to your [RecyclerView.Adapter]. It must implement
 * [StickyHeaders] to indicate which items are headers.
 */
class StickyHeadersLinearLayoutManager<T>(
    context: Context,
    @RecyclerView.Orientation orientation: Int = RecyclerView.VERTICAL,
    reverseLayout: Boolean = false
) : LinearLayoutManager(context, orientation, reverseLayout) where
...
T : StickyHeaders? {
    ...
    private fun scrollToPositionWithOffset(
        ...
        // Current sticky header is the same as at the position. Adjust the scroll offset and reset
        // pending scroll.
    ...
    /**
     * Updates the sticky header state (creation, binding, display), to be called whenever there's a
     * layout or scroll
     */
            ...
            if (anchorView != null && anchorPos != -1) {
                ...
                if (headerPos != -1 &&
                    ...
                    if (mStickyHeader != null &&
                        ...
                        // A sticky header was shown before but is not of the correct type. Scrap
                        // it.
                        scrapStickyHeader(recycler)
                    }

                    // Ensure sticky header is created, if absent, or bound, if being laid out or
                    ...
                    // Draw the sticky header using translation values which depend on orientation,
                    // direction and
                    ...
                    if (nextHeaderPos != -1) {
                        ...
                        // The header view itself is added to the RecyclerView. Discard it if it
                        // comes up.
    ...
    /**
     * Creates [RecyclerView.ViewHolder] for `position`, including measure / layout, and assigns it
     * to [.mStickyHeader].
     */
    private fun createStickyHeader(recycler: Recycler, position: Int) {
        ...
        // Add sticky header as a child view, to be detached / reattached whenever
        // LinearLayoutManager#fill() is called,
    ...
    /**
     * Returns [.mStickyHeader] to the [RecyclerView]'s [RecyclerView.RecycledViewPool], assigning
     * it to `null`.
     *
    ...
    /**
     * Returns the position in the Y axis to position the header appropriately, depending on
     * orientation, direction and [android.R.attr.clipToPadding].
     */
    ...
    /**
     * Returns the position in the X axis to position the header appropriately, depending on
     * orientation, direction and [android.R.attr.clipToPadding].
     */
    ...
    private fun findHeaderIndexOrBefore(position: Int): Int {
        ...
        while (low <= high) {
            ...
            if (mHeaderPositions[middle] > position) {
                high = middle - 1
            } else if (middle < mHeaderPositions.size - 1 &&
                mHeaderPositions[middle + 1] <= position) {
                low = middle + 1
            } else {
                return middle
            }
     ...
     *
     * This is used in detriment of [RecyclerView.LayoutManager]'s callbacks to control when they're
     * received.
     */
    private inner class HeaderPositionsAdapterDataObserver : AdapterDataObserver() {
        override fun onChanged() {
            ...
            // Remove sticky header immediately if the entry it represents has been removed. A
            // layout will follow.
            ...
            if (headerCount > 0) {
                ...
                while (i != -1 && i < headerCount) {
                    ...
                    if (headerPos >= fromPosition && headerPos < fromPosition + itemCount) {
                        ...
                        headerPos >= fromPosition + itemCount &&
