...
class RetryAwaitCaptchaSolutionEventHandler @Inject constructor() : EventHandler {
    override val event: KClass<out Event> = RetryAwaitCaptchaSolution::class

    override suspend fun invoke(
        state: State,
        event: Event,
    ): Next {
        val actualEvent = event as RetryAwaitCaptchaSolution
        return Next(
            nextState = state,
            sideEffect = AwaitCaptchaSolution(
                actionId = actualEvent.actionId,
                brokerName = actualEvent.brokerName,
                transactionID = actualEvent.transactionID,
