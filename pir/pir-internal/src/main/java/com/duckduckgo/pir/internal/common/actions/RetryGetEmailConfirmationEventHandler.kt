...
class RetryGetEmailConfirmationEventHandler @Inject constructor() : EventHandler {
    override val event: KClass<out Event> = RetryGetEmailConfirmation::class

    override suspend fun invoke(
        state: State,
        event: Event,
    ): Next {
        val actualEvent = event as RetryGetEmailConfirmation
        return Next(
            nextState = state,
            sideEffect = AwaitEmailConfirmation(
                actionId = actualEvent.actionId,
                brokerName = actualEvent.brokerName,
                extractedProfile = actualEvent.extractedProfile,
                pollingIntervalSeconds = actualEvent.pollingIntervalSeconds,
                attempt = actualEvent.attempt,
            ),
        )
    }
}
