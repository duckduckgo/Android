...
import com.duckduckgo.pir.internal.scan.PirUserProfile
...
/**
 * This class is responsible for executing a broker action and return the result of the action
 */
interface BrokerActionProcessor {
    ...
    /**
     * Executes the [action] for the given profile
     */
    fun pushAction(
        userProfile: PirUserProfile,
        action: BrokerAction,
        webView: WebView,
    )
...
class RealBrokerActionProcessor @Inject constructor(
    ...
    private val onActionReceivedParamAdapter = Moshi.Builder()
        .add(
    ...
    private var lastActionTaken: BrokerAction? = null
    ...
    override fun pushAction(
        ...
        SubscriptionEventData(
            ...
            params = onActionReceivedParamAdapter.toJson(
    ...
    private fun handleError(result: PirErrorReponse) {
        logcat { "PIR-CSS: handleError: result $result " }
        if (lastActionTaken?.id == result.actionID) {
            registeredActionResultListener?.onError(result)
        }
    }

    private fun handleSuccess(result: PirSuccessResponse) {
        logcat { "PIR-CSS: handleSuccess: result $result lastActionTaken: $lastActionTaken" }
        if (lastActionTaken?.id == result.actionID) {
    ...
    private fun processJsCallbackMessage(
        ...
        logcat { "PIR-CSS: processJsCallbackMessage: featureName $featureName method $method data $data " }
        if (method == PIRScriptConstants.RECEIVED_METHOD_NAME_COMPLETED && data != null) {
            val result = kotlin.runCatching {
                responseAdapter.fromJson(data.toString())
            }.onFailure {
                logcat { "PIR-CSS: processJsCallbackMessage: ERROR $it " }
            }.getOrNull()
            logcat { "PIR-CSS: processJsCallbackMessage: result $result " }
