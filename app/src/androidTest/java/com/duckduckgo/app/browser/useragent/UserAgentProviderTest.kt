...
class UserAgentProviderTest {
    ...
    fun whenUaRetrievedWithNoParamsThenDeviceStrippedAndApplicationComponentAddedBeforeSafari() {
        ...
        assertTrue(
            "$actual does not match expected regex", ValidationRegex.converted.matches(actual))
    }
    ...
    fun whenMissingAppleWebKitComponentThenUaContainsMozillaAndApplicationAndSafariComponents() {
        ...
        val actual = testee.userAgent(isDesktop = false)
        assertTrue(
            "$actual does not match expected regex", ValidationRegex.missingWebKit.matches(actual))
    }
    ...
    fun whenMissingSafariComponentThenUaContainsMozillaAndVersionAndApplicationComponents() {
        ...
        val actual = testee.userAgent(isDesktop = false)
        assertTrue(
            "$actual does not match expected result", ValidationRegex.missingSafari.matches(actual))
    }
    ...
    fun whenMissingVersionComponentThenUaContainsMozillaAndApplicationAndSafariComponents() {
        ...
        val actual = testee.userAgent(isDesktop = false)
        assertTrue(
            "$actual does not match expected result", ValidationRegex.noVersion.matches(actual))
    }
    ...
    fun whenDomainDoesNotSupportApplicationThenUaOmitsApplicationComponent() {
        ...
        assertTrue(
            "$actual does not match expected regex", ValidationRegex.noApplication.matches(actual))
    }
    ...
    fun whenDomainDoesNotSupportVersionThenUaOmitsVersionComponent() {
        ...
        assertTrue(
            "$actual does not match expected regex", ValidationRegex.noVersion.matches(actual))
    }
    ...
    // Some values will be dynamic based on OS/Architecture/Software versions, so we use Regex to
    // validate values
    private object ValidationRegex {
        ...
        val missingWebKit =
            Regex("Mozilla/5.0 \\(Linux; Android .*?\\) DuckDuckGo/5 Safari/[.0-9]+")
        val missingSafari =
            Regex(
