...
class Grade() {
    ...
    data class Score(val grade: Grading,
                     val score: Int,
                     val httpsScore: Int,
    ...
    data class Scores(val site: Score,
                      val enhanced: Score) {
    }
    ...
    private fun trackerScore(entities: Map<String, Double>): Int {
        return entities.entries.fold(0) { acc, entry ->
            ...
            val score = scoreFromPrevalence(entry.value)

            Timber.d("${entry} ${score}")
