...
class LocationPermissionsRepositoryTest {
    ...
    @Test
    fun whenPermissionIsValidThenItIsStored() =
        coroutineRule.runBlocking {
            val permissionType = LocationPermissionType.ALLOW_ALWAYS
            val permission = LocationPermissionEntity(domain, permissionType)
            val permissionStored =
                repository.savePermission(domain, LocationPermissionType.ALLOW_ALWAYS)
            assertEquals(permission, permissionStored)
        }

    @Test
    fun whenGetAllPermissionsAsyncThenReturnLiveDataWithAllItemsFromDatabase() =
        coroutineRule.runBlocking {
    ...
    @Test
    fun whenGetAllPermissionsSyncThenReturnListWithAllItemsFromDatabase() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com", "example2.com")
    ...
    @Test
    fun whenGetAllFireproofWebsitesThenReturnLiveDataWithAllItemsFromDatabase() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com", "example2.com")
            val entities = repository.getLocationPermissionsAsync().blockingObserve()!!
            assertEquals(entities.size, 2)
    ...
    @Test
    fun whenDeletePermissionStoredThenItemRemovedFromDatabase() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com", "example2.com")
    ...
    @Test
    fun whenDeletePermissionNotStoredThenNotingIsRemovedFromDatabase() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com", "example2.com")
    ...
    @Test
    fun whenRetrievingStoredPermissionThenItCanBeRetrieved() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com", "example2.com")
    ...
    @Test
    fun whenRetrievingNotStoredPermissionThenNothingCanBeRetrieved() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com", "example2.com")
    ...
    @Test
    fun whenDeletePermissionStoredThenDeletePersistedFavicon() =
        coroutineRule.runBlocking {
    ...
    @Test
    fun whenPermissionEntitiesCountByDomainAndNoWebsitesMatchThenReturnZero() =
        coroutineRule.runBlocking {
            givenPermissionStored("example.com")
    ...
    @Test
    fun whenPermissionEntitiesCountByDomainAndWebsitsMatchThenReturnCount() =
        coroutineRule.runBlocking {
            val query = "%example%"
            givenPermissionStored("example.com")
        ...
        domains.forEach {
            dao.insert(
                LocationPermissionEntity(
                    domain = it, permission = LocationPermissionType.ALLOW_ALWAYS))
        }
    }
}
