...
class PlayHttpsEmbeddedDataPersister(
    ...
    override fun shouldPersistEmbeddedData(): Boolean {
        ...
        return !binaryDataStore.verifyCheckSum(
    ...
    override fun persistEmbeddedData() {
        ...
        val specJson =
            context
                .resources
                .openRawResource(R.raw.https_mobile_v2_bloom_spec)
                .bufferedReader()
                .use { it.readText() }
        val specAdapter = moshi.adapter(HttpsBloomFilterSpec::class.java)

        val falsePositivesJson =
            context
                .resources
                .openRawResource(R.raw.https_mobile_v2_false_positives)
                .bufferedReader()
                .use { it.readText() }
        val falsePositivesType =
            Types.newParameterizedType(List::class.java, HttpsFalsePositiveDomain::class.java)
        val falsePositivesAdapter: JsonAdapter<List<HttpsFalsePositiveDomain>> =
            moshi.adapter(falsePositivesType)

        val bytes =
            context.resources.openRawResource(R.raw.https_mobile_v2_bloom).use { it.readBytes() }
        httpsDataPersister.persistBloomFilter(
            specAdapter.fromJson(specJson)!!,
            bytes,
            falsePositivesAdapter.fromJson(falsePositivesJson)!!)
    }
}
