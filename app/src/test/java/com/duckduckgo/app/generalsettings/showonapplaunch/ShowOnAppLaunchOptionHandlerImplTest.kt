...
class ShowOnAppLaunchOptionHandlerImplTest {
    ...
    fun setup() {
        ...
        testee = ShowOnAppLaunchOptionHandlerImpl(dispatcherProvider, fakeDataStore, fakeTabRepository)
    }
    ...
    @Test
    fun whenOptionIsSpecificUrlWithDomainOnlyAndTabAlreadyAddedWithSchemeAndDomainThenTabIsNotAdded() =
        runTest {
            fakeDataStore.setShowOnAppLaunchOption(SpecificPage("example.com"))
    ...
    private class FakeTabRepository : TabRepository {
        ...
        override suspend fun findTabIdByUrlPattern(urlPattern: String): String? =
            tabs.entries.findLast { it.value.contains(Regex(".*$urlPattern.*")) }?.key?.toString()
