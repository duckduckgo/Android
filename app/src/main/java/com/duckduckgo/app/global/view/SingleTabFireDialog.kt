...
private const val ARG_IS_FROM_TAB_SWITCHER = "isFromTabSwitcher"
...
class SingleTabFireDialog : BottomSheetDialogFragment(), FireDialog {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val isFromTabSwitcher = arguments?.getBoolean(ARG_IS_FROM_TAB_SWITCHER, false) ?: false
        viewModel.setFromTabSwitcher(isFromTabSwitcher)
    ...
    private fun handleCommand(command: Command) {
        when (command) {
            ...
            is Command.OnSingleTabClear -> {
                sendFragmentResult(FireDialog.EVENT_ON_SINGLE_TAB_CLEAR)
                dismiss()
            }
        }
    }
    ...
    private fun render(state: SingleTabFireDialogViewModel.ViewState) {
        val titleRes = if (state.isDuckAiChatsSelected) {
            com.duckduckgo.app.browser.R.string.singleTabFireDialogTitleWithChats
        } else {
            com.duckduckgo.app.browser.R.string.singleTabFireDialogTitle
        }
        ...
        if (state.isDuckAiTab) {
            binding.dialogSubtitle.text = requireContext().getString(
                com.duckduckgo.app.browser.R.string.singleTabFireDialogSubtitleDuckAi,
            )
    ...
    companion object {
        fun newInstance(isFromTabSwitcher: Boolean = false): SingleTabFireDialog {
            return SingleTabFireDialog().apply {
                arguments = bundleOf(ARG_IS_FROM_TAB_SWITCHER to isFromTabSwitcher)
            }
        }
    }
}
