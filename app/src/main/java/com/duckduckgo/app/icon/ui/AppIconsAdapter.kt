...
class AppIconsAdapter(private val onClick: (ChangeIconViewModel.IconViewData) -> Unit) : RecyclerView.Adapter<AppIconsAdapter.IconViewHolder>() {
    ...
    private fun View.applySelectionOutline() {
        val strokeWidth = resources.getDimension(R.dimen.changeAppIconSelectionStrokeWidth)
        val gap = resources.getDimension(R.dimen.changeAppIconSelectionGap)
        val inset = (strokeWidth + gap).roundToInt()
