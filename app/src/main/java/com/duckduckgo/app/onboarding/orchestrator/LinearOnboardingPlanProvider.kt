...
// Plan composition only. Terminal side-effects (AppStage write / skipper) are
// injected as lambdas by the lifecycle owner — keeping this class free of
// dependencies on UserStageStore / OnboardingSkipper.
...
class LinearOnboardingPlanProvider @Inject constructor(
    ...
    // Cross-step context kept private to the provider. Steps close over it.
    private val context = MutableStateFlow(OnboardingPocContext())

    // Side plan reached via SwitchTo from initial_reinstall_user. Its AbortPlan
    // bubbles up to the main plan's onSkipped (per orchestrator semantics).
