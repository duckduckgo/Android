...
class SenseOfProtectionExperimentImpl @Inject constructor(
    ...
    override fun isEnabled(cohort: CohortName): Boolean {
        ...
        return if (userBrowserProperties.daysSinceInstalled() > EXISTING_USER_DAY_COUNT_THRESHOLD) {
            ...
            if (isNotEnrolledInNewUserExperiment()) {
                isExistingUserExperimentEnabled()
            } else {
                false
            }
        } else {
            isNewUserExperimentEnabled()
        }
    }
    ...
    private fun isNewUserExperimentEnabled(): Boolean =
        senseOfProtectionToggles.senseOfProtectionNewUserExperimentApr25().isEnabled(cohort = MODIFIED_CONTROL)

    private fun isExistingUserExperimentEnabled(): Boolean =
        senseOfProtectionToggles.senseOfProtectionExistingUserExperimentApr25().isEnabled(cohort = MODIFIED_CONTROL)
}
