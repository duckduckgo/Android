...
private val twoFactorAuthUrlPatterns =
    mapOf<String, Set<Pattern>>(
        Pair("accounts.google.com", setOf(Pattern.compile("signin/v\\d.*/challenge"))),
        Pair("sso", setOf(Pattern.compile("duosecurity/getduo"))),
        Pair(
            "amazon.com",
            setOf(Pattern.compile("ap/challenge"), Pattern.compile("ap/cvf/approval"))))

private var ssoUrlPatterns =
    mapOf<String, Set<Pattern>>(Pair("sso", setOf(Pattern.compile("saml2/idp/SSOService"))))

// Following patterns have been extracted from each domain oauth official docs
private var oAuthUrlPatterns =
    mapOf<String, Set<Pattern>>(
        Pair(
            "accounts.google.com",
            setOf(Pattern.compile("o/oauth2/auth"), Pattern.compile("o/oauth2/v\\d.*/auth"))),
        ...
        Pair(
            "facebook.com",
            setOf(Pattern.compile("/v\\d.*/dialog/oauth"), Pattern.compile("dialog/oauth"))),
        Pair(
            "login.microsoftonline.com",
            setOf(
                Pattern.compile("common/oauth2/authorize"),
                Pattern.compile("common/oauth2/v2.0/authorize"))),
        ...
        Pair(
            "api.twitter.com",
            setOf(Pattern.compile("oauth/authenticate"), Pattern.compile("oauth/authorize"))),
