...
class RealNativeInputAnimator : NativeInputAnimator {
    ...
    override fun animateExit(
        ...
        val targetWidth = omnibarCard.width
        val targetHeight = omnibarCard.height

        waitForLayout(widgetCard) {
            val postPosition = windowPosition(widgetCard)
            val holdX = (preSurface.x - postPosition.x).toFloat()
            val holdY = (preSurface.y - postPosition.y).toFloat()
            widgetCard.translationX = holdX
            widgetCard.translationY = holdY

            val bottomAnchorShift = if (isBottom) visibleBounds.height - targetHeight else 0
            val endTranslationX = (omnibarPosition.x - postPosition.x).toFloat()
            val endTranslationY = (omnibarPosition.y - postPosition.y).toFloat() - bottomAnchorShift
            ...
            runAnimator(
                ...
                onUpdate = { fraction ->
                    params.width = lerp(visibleBounds.width, targetWidth, fraction)
                    params.height = lerp(visibleBounds.height, targetHeight, fraction)
                    widgetCard.layoutParams = params

                    widgetCard.translationX = lerpF(holdX, endTranslationX, fraction)
                    widgetCard.translationY = lerpF(holdY, endTranslationY, fraction)
    ...
    private fun performEntranceAnimation(
        card: View,
        omnibarCard: View,
        widgetView: View,
        margins: Margins,
