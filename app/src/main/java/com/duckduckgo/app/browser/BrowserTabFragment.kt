...
class BrowserTabFragment :
    ...
    private val autofillCallback = object : Callback {
        override suspend fun onCredentialsAvailableToInject(
            ...
            withContext(dispatchers.main()) {
                showAutofillDialogChooseCredentials(autofillUrlRequest, credentials, triggerType)
            }
        }
        ...
        override suspend fun onGeneratedPasswordAvailableToUse(
            ...
            withContext(dispatchers.main()) {
                showUserAutoGeneratedPasswordDialog(autofillUrlRequest, username, generatedPassword)
            }
        }
        ...
        override fun showNativeChooseEmailAddressPrompt(autofillUrlRequest: AutofillUrlRequest) {
            viewModel.showEmailProtectionChooseEmailPrompt(autofillUrlRequest)
        }
        ...
        override suspend fun onCredentialsAvailableToSave(
            ...
            val currentUrl = autofillUrlRequest.requestOrigin
            ...
            withContext(dispatchers.main()) {
                when (matchType) {
                    ...
                    UsernameMatch -> showAutofillDialogUpdatePassword(autofillUrlRequest, credentials)
                    UsernameMissing -> showAutofillDialogUpdateUsername(autofillUrlRequest, credentials)
                    NoMatch -> showAutofillDialogSaveCredentials(autofillUrlRequest, credentials)
                    UrlOnlyMatch -> showAutofillDialogSaveCredentials(autofillUrlRequest, credentials)
                }
            }
        }
        ...
        private fun showUserAutoGeneratedPasswordDialog(
            ...
            val url = webView?.url ?: return
            if (url != autofillUrlRequest.originalPageUrl) {
                Timber.w("WebView url has changed since autofill request; bailing")
                return
            }
            val dialog = credentialAutofillDialogFactory.autofillGeneratePasswordDialog(autofillUrlRequest, username, generatedPassword, tabId)
            showDialogHidingPrevious(dialog, UseGeneratedPasswordDialog.TAG, autofillUrlRequest.originalPageUrl)
        }
        ...
        private fun showAutofillDialogChooseCredentials(
            ...
            val dialog = credentialAutofillDialogFactory.autofillSelectCredentialsDialog(autofillUrlRequest, credentials, triggerType, tabId)
            showDialogHidingPrevious(dialog, CredentialAutofillPickerDialog.TAG, autofillUrlRequest.originalPageUrl)
        }
    }
    ...
    override fun onSelectedToSignUpForInContextEmailProtection(autofillUrlRequest: AutofillUrlRequest) {
        showEmailProtectionInContextWebFlow(autofillUrlRequest = autofillUrlRequest)
    }
    ...
    private fun processCommand(it: Command?) {
        ...
        when (it) {
            ...
            is Command.ShowEmailProtectionChooseEmailPrompt -> showEmailProtectionChooseEmailDialog(it.address, it.autofillUrlRequest)
            is Command.NotifyAutofillPageChanged -> notifyAutofillPageChanged()
    ...
    private fun notifyAutofillPageChanged() {
    ...
    private fun showAutofillDialogSaveCredentials(
        ...
        val dialog = credentialAutofillDialogFactory.autofillSavingCredentialsDialog(autofillUrlRequest, credentials, tabId)
    ...
    private fun showAutofillDialogUpdatePassword(
        ...
        val dialog = credentialAutofillDialogFactory.autofillSavingUpdatePasswordDialog(autofillUrlRequest, credentials, tabId)
    ...
    private fun showAutofillDialogUpdateUsername(
        ...
        val dialog = credentialAutofillDialogFactory.autofillSavingUpdateUsernameDialog(autofillUrlRequest, credentials, tabId)
        showDialogHidingPrevious(dialog, CredentialUpdateExistingCredentialsDialog.TAG)
    }
    ...
    private fun showEmailProtectionInContextWebFlow(autofillUrlRequest: AutofillUrlRequest) {
        context?.let {
            val params = EmailProtectionInContextSignUpStartScreen(messageRequestId = autofillUrlRequest.requestId)
            val intent = globalActivityStarter.startIntent(it, params)
            activityResultHandlerEmailProtectionInContextSignup.launch(intent)
            inContextEmailProtectionSignupState = InProgressEmailProtectionSignupState(showing = true, requestId = autofillUrlRequest.requestId)
        }
    }
