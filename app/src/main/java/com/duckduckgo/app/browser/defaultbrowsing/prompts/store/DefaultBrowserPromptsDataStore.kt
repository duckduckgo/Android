...
class DefaultBrowserPromptsPrefsDataStoreImpl @Inject constructor(
    ...
    override val showOverflowMenuItem: Flow<Boolean> = store.data.map { preferences ->
        preferences[booleanPreferencesKey(PREF_KEY_SHOW_OVERFLOW_MENU_ITEM)] ?: false
    }

    override val highlightOverflowMenuIcon: Flow<Boolean> = store.data.map { preferences ->
