...
class DataUriDownloader @Inject constructor(private val dataUriParser: DataUriParser) {
    ...
    fun download(
        ...
        try {
            ...
            when (val parsedDataUri = dataUriParser.generate(pending.url)) {
                is ParseResult.Invalid -> {
                    ...
                    callback?.downloadFailed(
                        "Failed to extract data from data URI",
                        DownloadFailReason.DataUriParseException)
                    return
                }
                is ParseResult.ParsedDataUri -> {
                    val file = initialiseFilesOnDisk(pending, parsedDataUri.filename)

                    runCatching { writeBytesToFiles(parsedDataUri.data, file) }
                        .onSuccess {
            ...
            callback?.downloadFailed(
                "Failed to download data uri", DownloadFailReason.DataUriParseException)
        }
    }
