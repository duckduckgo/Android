...
class FeedbackViewModel(
    ...
    init {
        updateViewCommand.postValue(
            UpdateViewCommand(
                fragmentViewState = InitialAppEnjoymentClarifier(NAVIGATION_FORWARDS)))
    }
    ...
    fun userSelectedNegativeFeedbackMainReason(mainReason: MainReason) {
        val newState =
            when (mainReason) {
                MainReason.MISSING_BROWSING_FEATURES ->
                    NegativeFeedbackSubReason(NAVIGATION_FORWARDS, mainReason)
                MainReason.WEBSITES_NOT_LOADING ->
                    NegativeWebSitesBrokenFeedback(NAVIGATION_FORWARDS, mainReason)
                MainReason.SEARCH_NOT_GOOD_ENOUGH ->
                    NegativeFeedbackSubReason(NAVIGATION_FORWARDS, mainReason)
                MainReason.NOT_ENOUGH_CUSTOMIZATIONS ->
                    NegativeFeedbackSubReason(NAVIGATION_FORWARDS, mainReason)
                MainReason.APP_IS_SLOW_OR_BUGGY ->
                    NegativeFeedbackSubReason(NAVIGATION_FORWARDS, mainReason)
    ...
    fun onBackPressed() {
        ...
        when (currentViewState.fragmentViewState) {
            ...
            is PositiveFeedbackFirstStep -> {
                updateViewCommand.value =
                    currentViewState.copy(
                        fragmentViewState = InitialAppEnjoymentClarifier(NAVIGATION_BACKWARDS))
            }
            is PositiveShareFeedback -> {
                ...
                if (canShowRatingsButton()) {
                    updateViewCommand.value =
                        currentViewState.copy(
                            fragmentViewState = PositiveFeedbackFirstStep(NAVIGATION_BACKWARDS))
                } else {
                    updateViewCommand.value =
                        currentViewState.copy(
                            fragmentViewState = InitialAppEnjoymentClarifier(NAVIGATION_BACKWARDS))
            ...
            is NegativeFeedbackSubReason -> {
                updateViewCommand.value =
                    currentViewState.copy(
                        fragmentViewState = NegativeFeedbackMainReason(NAVIGATION_BACKWARDS))
            }
            ...
            is NegativeOpenEndedFeedback -> {
                shouldHideKeyboard = true
                val newViewState =
                    when (currentViewState.previousViewState) {
                        is NegativeFeedbackSubReason -> {
                            NegativeFeedbackSubReason(
                                NAVIGATION_BACKWARDS, currentViewState.mainReason!!)
                        }
                        is NegativeFeedbackMainReason -> {
    ...
    private fun canShowRatingsButton(): Boolean {
        ...
        if (BuildConfig.DEBUG) {
            Timber.i(
                "Not installed from the Play Store but it is DEBUG; will treat as if installed from Play Store")
            return true
        }
        return false
    }
    ...
    fun userGavePositiveFeedbackNoDetails() {
        appCoroutineScope.launch(dispatchers.main()) {
            command.value = Exit(feedbackSubmitted = true)
            withContext(dispatchers.io()) { feedbackSubmitter.sendPositiveFeedback(null) }
        }
    }
    ...
    fun userSelectedToRateApp() {
        appCoroutineScope.launch(dispatchers.main()) {
            command.value = Exit(feedbackSubmitted = true)
            appCoroutineScope.launch(dispatchers.io()) { feedbackSubmitter.sendUserRated() }
        }
    }
    ...
    fun userProvidedPositiveOpenEndedFeedback(feedback: String) {
        appCoroutineScope.launch(dispatchers.main()) {
            command.value = Exit(feedbackSubmitted = true)
            withContext(dispatchers.io()) { feedbackSubmitter.sendPositiveFeedback(feedback) }
        }
    }
...
sealed class FragmentState(open val forwardDirection: Boolean) {
    data class InitialAppEnjoymentClarifier(override val forwardDirection: Boolean) :
        FragmentState(forwardDirection)

    // positive flow
    data class PositiveFeedbackFirstStep(override val forwardDirection: Boolean) :
        FragmentState(forwardDirection)

    data class PositiveShareFeedback(override val forwardDirection: Boolean) :
        FragmentState(forwardDirection)

    // negative flow
    data class NegativeFeedbackMainReason(override val forwardDirection: Boolean) :
        FragmentState(forwardDirection)
        ...
        with(modelClass) {
            return when {
                isAssignableFrom(FeedbackViewModel::class.java) ->
                    (FeedbackViewModel(
                        playStoreUtils.get(), feedbackSubmitter.get(), appCoroutineScope.get()) as
                        T)
                else -> null
            }
        }
