...
abstract class TdsDomainEntityDao {
    ...
    @Query("select * from tds_domain_entity") abstract fun getAll(): List<TdsDomainEntity>

    @Query("select * from tds_domain_entity where domain=:domain")
    abstract fun get(domain: String): TdsDomainEntity?

    @Query("delete from tds_domain_entity") abstract fun deleteAll()

    @Query("select count(*) from tds_domain_entity") abstract fun count(): Int
