...
class WebTrackersBlockedAppRepository @Inject constructor(appDatabase: AppDatabase) :
    ...
    override fun get(startTime: () -> String, endTime: String): Flow<List<WebTrackerBlocked>> {
        return dao.getTrackersBetween(startTime(), endTime).distinctUntilChanged().map {
            it.filter { tracker -> tracker.timestamp >= startTime() }
        }
    }
}
