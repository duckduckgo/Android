...
/**
 * Stores information about unsent automatic data clearer restart Pixels, detecting if user started
 * the app from an external Intent. Contains logic to send unsent pixels.
 *
...
@Inject
constructor(private val context: Context, private val pixel: Pixel) : LifecycleObserver {
    ...
    fun incrementCount() {
        if (detectedUserIntent) {
            ...
            incrementCount(
                pendingAppForegroundRestart, KEY_UNSENT_CLEAR_APP_RESTARTED_WITH_INTENT_PIXELS)
        } else {
            Timber.i("Registered restart without intent")
            incrementCount(
                pendingAppForegroundRestartWithIntent, KEY_UNSENT_CLEAR_APP_RESTARTED_PIXELS)
        }
    }
    ...
    fun firePendingPixels() {
        ...
        firePendingPixels(
            pendingAppForegroundRestartWithIntent, AppPixelName.FORGET_ALL_AUTO_RESTART_WITH_INTENT)
        resetCount()
    }

    private fun incrementCount(counter: Int, sharedPrefKey: String) {
        val updated = counter + 1
        preferences.edit(commit = true) { putInt(sharedPrefKey, updated) }
    }
    ...
    private fun widgetActivity(intent: Intent?): Boolean =
        intent?.component?.className?.contains(
            SystemSearchActivity::class.java.canonicalName.orEmpty()) == true
    ...
    companion object {
        ...
        const val KEY_UNSENT_CLEAR_APP_RESTARTED_WITH_INTENT_PIXELS =
            "KEY_UNSENT_CLEAR_APP_RESTARTED_WITH_INTENT_PIXELS"
    }
}
