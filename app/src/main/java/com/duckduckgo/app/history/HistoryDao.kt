...
interface HistoryDao {
    ...
    fun updateOrInsertVisit(url: String, title: String, query: String?, isSerp: Boolean, date: Long) {
        ...
        if (existingHistoryEntry != null) {
            val newVisit = Visit(date = date, historyEntryId = existingHistoryEntry.id)
            insertVisit(newVisit)
        } else {
            val newHistoryEntry = HistoryEntry(url = url, title = title, query = query, isSerp = isSerp)
            val historyEntryId = insertHistoryEntry(newHistoryEntry)

            val newVisit = Visit(date = date, historyEntryId = historyEntryId)
            insertVisit(newVisit)
        }
    }

    @Query("SELECT * FROM history_entries WHERE url = :url LIMIT 1")
    fun getHistoryEntryByUrl(url: String): HistoryEntry?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertHistoryEntry(historyEntry: HistoryEntry): Long
