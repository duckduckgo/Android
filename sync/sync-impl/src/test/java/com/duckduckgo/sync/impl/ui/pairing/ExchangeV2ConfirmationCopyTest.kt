...
class ExchangeV2ConfirmationCopyTest {

    @Test fun whenKnownDdgPeerThenFullDataList() {
        assertEquals(
            R.string.sync_v2_confirmation_message_ddg,
            exchangeV2ConfirmationMessageRes(unknownPeer = false, peerKind = PeerKind.DDG),
        )
    }
    ...
    @Test fun whenKnownThirdPartyPeerThenChatsOnly() {
        assertEquals(
            ...
            exchangeV2ConfirmationMessageRes(unknownPeer = false, peerKind = PeerKind.THIRD_PARTY),
        )
    }
    ...
    @Test fun whenUnknownDdgPeerThenFullDataListWithoutName() {
        assertEquals(
            ...
            exchangeV2ConfirmationMessageRes(unknownPeer = true, peerKind = PeerKind.DDG),
        )
    }
    ...
    @Test fun whenUnknownThirdPartyPeerThenChatsOnlyWithoutName() {
        assertEquals(
            ...
            exchangeV2ConfirmationMessageRes(unknownPeer = true, peerKind = PeerKind.THIRD_PARTY),
        )
    }
    ...
    @Test fun whenKindNullThenFallsBackToFullDdgList() {
        assertEquals(
            R.string.sync_v2_confirmation_message_ddg,
            exchangeV2ConfirmationMessageRes(unknownPeer = false, peerKind = null),
        )
        assertEquals(
            R.string.sync_v2_confirmation_message_unknown_peer_ddg,
            exchangeV2ConfirmationMessageRes(unknownPeer = true, peerKind = null),
        )
    }
}
