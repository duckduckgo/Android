...
class SyncedDeviceAdapter(
    ...
    override fun getItemViewType(position: Int): Int = when (val item = getItem(position)) {
        ...
        is SyncedDevice -> when {
            ...
            item.device.thisDevice -> R.layout.item_sync_device_local
            else -> R.layout.item_sync_device_remote
        }
    }
    ...
    override fun onCreateViewHolder(
        ...
        return when (viewType) {
            R.layout.item_sync_device_local -> {
                LocalDeviceViewHolder(
                    ItemSyncDeviceLocalBinding.inflate(inflater, parent, false),
                    listener,
                )
            }

            R.layout.item_sync_device_remote -> {
                RemoteDeviceViewHolder(
                    ItemSyncDeviceRemoteBinding.inflate(inflater, parent, false),
                    listener,
                )
            }
...
private class LocalDeviceViewHolder(
    private val binding: ItemSyncDeviceLocalBinding,
    private val listener: Listener,
...
private class RemoteDeviceViewHolder(
    private val binding: ItemSyncDeviceRemoteBinding,
    private val listener: Listener,
) : ViewHolder(binding.root) {
    private var device: ConnectedDevice? = null
