...
class RecoverDataViewModel @Inject constructor(
    ...
    private fun createAccount() = viewModelScope.launch(dispatchers.io()) {
        val restoreEnabled = syncAutoRestoreManager.isRestoreOnReinstallEnabled()
        val showRestore = syncFeature.syncAutoRestore().isEnabled() &&
            persistentStorage.checkAvailability() is PersistentStorageAvailability.Available

        if (!syncAccountRepository.isSignedIn()) {
            viewState.emit(ViewState(viewMode = CreatingAccount, showRestoreOnReinstall = showRestore, restoreOnReinstallEnabled = restoreEnabled))
        ...
        syncAccountRepository.getRecoveryCode().getOrNull()?.let { recoveryCode ->
            viewState.emit(
                ViewState(
                    ...
                    restoreOnReinstallEnabled = restoreEnabled,
                ),
            )
