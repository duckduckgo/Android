...
class SyncSetupIntroFragment : DuckDuckGoFragment(R.layout.fragment_intro_sync) {
    ...
    private fun setupEdgeToEdgeInsets() {
        ...
        val footerMargin = resources.getDimensionPixelSize(com.duckduckgo.mobile.android.R.dimen.keyline_4)
        ViewCompat.setOnApplyWindowInsetsListener(binding.syncIntroFooter) { view, windowInsets ->
            val insets = windowInsets.getInsets(
                WindowInsetsCompat.Type.systemBars() or WindowInsetsCompat.Type.displayCutout(),
            )
            view.updateLayoutParams<MarginLayoutParams> {
                bottomMargin = footerMargin + insets.bottom
            }
            windowInsets
        }
    }
