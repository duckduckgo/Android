...
function unblurImageOnLoad(image) {
    try {
        ...
        SafeGazeInterface.sendMessage("count")
    } catch (error) {
...
async function replaceImagesWithApiResults(apiUrl = 'https://api.safegaze.com/api/v1/analyze') {
    try {
        ...
        fetchNewImages();
        window.addEventListener('load', fetchNewImages);
        window.addEventListener('scroll', fetchNewImages);
        window.addEventListener('unload', SafeGazeInterface.sendMessage("page_refresh"))
    } catch (error) {
