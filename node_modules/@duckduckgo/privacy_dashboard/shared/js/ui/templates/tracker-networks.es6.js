...
function renderSummary (model) {
    ...
    if (model.site.isBroken && !model.site.isDenylisted) {
        return i18n.t('site:trackersFoundForAllowlisted.title')
    }
...
function renderTrackerDetails (model) {
    ...
    const blockedCompanies = reduceCompanyListByUrl(companyList, (company, url) => {
        ...
        return !model.site.protectionsEnabled ? !sameEntity : (!sameEntity && company.urls[url].isBlocked)
    })
    const unblockedCompanies = reduceCompanyListByUrl(companyList, (company, url) => isSameEntity(company, model.tab.parentEntity))
    const count = model.site.protectionsEnabled ? model.site.trackersBlockedCount : model.site.trackersCount
    ...
    return bel`
        ...
        unblockedCompanies.map(renderCompanyTrackers(model))
            ...
            ${trackerListWrapper(
        i18n.t('site:trackerCountForDomain.title', { trackerCount: count, blocked: model.site.protectionsEnabled, domain: model.domain }),
