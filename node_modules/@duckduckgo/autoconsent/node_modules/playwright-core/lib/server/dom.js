...
class ElementHandle extends js.JSHandle {
  ...
  async _performPointerAction(progress, actionName, waitForEnabled, action, forceScrollOptions, options) {
    ...
    if (!options.force) {
      ...
      if (hitPoint === 'error:notconnected') return hitPoint;
  ...
  async _setInputFiles(progress, items, options) {
    ...
    if (files) {
      for (const payload of files) {
        if (!payload.mimeType) payload.mimeType = _utilsBundle.mime.getType(payload.name) || 'application/octet-stream';
      }
    }
  ...
  async _viewportPointToDocument(point) {
    if (!this._frame.parentFrame()) return point;
