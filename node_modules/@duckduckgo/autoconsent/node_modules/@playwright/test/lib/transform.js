...
function transformHook(code, filename, moduleUrl) {
  ...
  if (!isTypeScript && !isJSX && !hasPreprocessor) return code;
...
function installTransform() {
  ...
  const exts = ['.ts', '.tsx', '.jsx']; // When script preprocessor is engaged, we transpile JS as well.

  if (scriptPreprocessor) exts.push('.js', '.mjs');

  const revertPirates = _utilsBundle.pirates.addHook((code, filename) => transformHook(code, filename), {
    exts
  });
