...
class TestTypeImpl {
  ...
  _ensureCurrentSuite(location, title) {
    const suite = (0, _globals.currentlyLoadingFileSuite)();

    if (!suite) {
      throw (0, _util.errorWithLocation)(location, [`Playwright Test did not expect ${title} to be called here.`, `Most common reasons include:`, `- You are calling ${title} in a configuration file.`, `- You are calling ${title} in a file that is imported by the configuration file.`, `- You have two different versions of @playwright/test. This usually happens`, `  when one of the dependencies in your package.json depends on @playwright/test.`].join('\n'));
    }
  ...
  _createTest(type, location, title, fn) {
    ...
    if (type === 'skip' || type === 'fixme') test.expectedStatus = 'skipped';

    for (let parent = suite; parent; parent = parent.parent) {
      if (parent._skipped) test.expectedStatus = 'skipped';
    }
  }
  ...
  _describe(type, location, title, fn) {
    ...
    if (type === 'skip') child._skipped = true;
  ...
  _configure(location, options) {
    ...
    if (suite._parallelMode !== 'default') throw (0, _util.errorWithLocation)(location, 'Parallel mode is already assigned for the enclosing scope.');
    suite._parallelMode = options.mode;

    for (let parent = suite.parent; parent; parent = parent.parent) {
      if (parent._parallelMode === 'serial' && suite._parallelMode === 'parallel') throw (0, _util.errorWithLocation)(location, 'describe.parallel cannot be nested inside describe.serial');
    }
  }
  ...
  _extendTest(location, test) {
    ...
    if (!testTypeImpl) throw new Error(`test._extendTest() accepts a single "test" parameter.\nDid you mean to call test.extend() with fixtures instead?`); // Filter out common ancestor fixtures.
