...
class DotReporter extends _base.BaseReporter {
  ...
  onTestEnd(test, result) {
    ...
    if (result.status === 'skipped') {
      process.stdout.write(_utilsBundle.colors.yellow('°'));
      return;
    }

    if (this.willRetry(test)) {
      process.stdout.write(_utilsBundle.colors.gray('×'));
      return;
    }

    switch (test.outcome()) {
      case 'expected':
        process.stdout.write(_utilsBundle.colors.green('·'));
        break;

      case 'unexpected':
        process.stdout.write(_utilsBundle.colors.red(result.status === 'timedOut' ? 'T' : 'F'));
        break;
